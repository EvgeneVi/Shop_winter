// import React from 'react'
import "./SendMail.scss";

export default function SendMail({ theme }) {
  let SendMail = (cls, txt = "Email") => {
    return (
      <form className={`send-mail ${cls}`} action="#" name="email">
        <i>
          <svg
            width="20"
            height="16"
            viewBox="0 0 20 16"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
          >
            <path
              d="M18 4L10 9L2 4V2L10 7L18 2V4ZM18 0H2C0.89 0 0 0.89 0 2V14C0 14.5304 0.210714 15.0391 0.585786 15.4142C0.960859 15.7893 1.46957 16 2 16H18C18.5304 16 19.0391 15.7893 19.4142 15.4142C19.7893 15.0391 20 14.5304 20 14V2C20 1.46957 19.7893 0.960859 19.4142 0.585786C19.0391 0.210714 18.5304 0 18 0Z"
              fill="black"
            />
          </svg>
        </i>
        <input type="text" name="adress" placeholder=""></input>
        <span data-mobiletxt="Введите имейл" className="send-mail__placeholder">
          {txt}
        </span>
        <i>
          <svg
            width="22"
            height="19"
            viewBox="0 0 22 19"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
          >
            <path
              d="M0 19V11.6276L15.7143 9.5L0 7.3724V0L22 9.5L0 19Z"
              fill="black"
            />
          </svg>
        </i>
      </form>
    );
  };

  switch (theme) {
    case "dark":
      SendMail = SendMail("dark", "");
      break;
    case "bright":
      SendMail = SendMail("bright");
      break;
    default:
      SendMail = SendMail("bright");
  }

  return SendMail;
}
